...
class DataLoader:
    ...
    def load_blendshape(self):
        ...
        for file in self.file_list:
            ...
            # print("file:{}\tshape:{}".format(blendshape_file, blendshape.shape))
    ...
    def get_all_blendshape(self):
        BS = np.vstack(self.blendshape)
        plt.figure("statistic")
        for i in range(51):
            plt.subplot(3, 17, i+1)
            print(np.histogram(BS[:, i], range=[0, 100]))
            plt.hist(BS[:, i], range=[0, 100], bins=100)
            plt.yticks([])
        plt.show()
    def statistic_database(self):
        print("database\ttotal samples：{}".format(self.total_num))
        length = np.unique(self.length)
        count = []

        for leng in length:
            count.append(self.length.count(leng))
        print("database\tlength mean: {}".format(np.mean(length)))
        print("database\tlength std: {}".format(np.mean(length)))
        ...
        self.get_all_blendshape()

if __name__ == "__main__":
    test_data = DataLoader("./char2index.txt",
                            "./train_list.txt",
                            "./train_data.txt",
                            "../data/spDB")
